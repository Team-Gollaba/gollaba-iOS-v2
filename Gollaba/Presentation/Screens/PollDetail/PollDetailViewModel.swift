...
class PollDetailViewModel {
    ...
    //MARK: - UseCase
    private let votingUseCase: VotingUseCaseProtocol
    ...
    //MARK: - API
    func getPoll() async {
        switch await votingUseCase.getPoll(pollHashId: self.id) {
        case .success(let newPoll):
            ...
            if let poll {
                ...
                handleError(error: .requestFailed(PollError.pollNotFound.rawValue))
            }
    ...
    func readPoll() async {
        if case .failure(let error) = await votingUseCase.readPoll(pollHashId: self.id) {
            handleError(error: error)
        }
    }
    ...
    func votingCheck() async {
        if let poll {
            switch await votingUseCase.votingCheck(pollHashId: poll.id) {
            case .success(let isVoted):
    ...
    func getVotingId() async {
        ...
        if let poll {
            switch await votingUseCase.getVotingId(pollHashId: poll.id) {
            case .success(let data):
                self.votingIdData = data
            case .failure(let error):
                handleError(error: error)
            }
        } else {
    ...
    func getPollVoters() async {
        if let poll {
            ...
            switch await votingUseCase.getVoters(pollHashId: poll.id) {
            case .success(let voters):
                self.pollVoters = voters
            case .failure(let error):
                handleError(error: error)
            }
        } else {
    ...
    func vote() async {
        ...
        let result = await votingUseCase.vote(pollHashId: self.id, pollItemIds: pollItemIds, voterName: poll.pollType == PollType.named.rawValue ? voterName : nil)
        switch result {
        case .success:
            self.isVoted = true
        ...
        case .failure(let error):
            if case .requestFailed(let status) = error, status == NetworkError.alreadyVotedStatus {
    ...
    func updateVote() async {
        ...
        switch await votingUseCase.updateVote(votingId: votingIdData.votingId, voterName: voterName, pollItemIds: pollItemIds) {
        case .success:
            self.activateSelectAnimation = true
            self.votingButtonState = .notChanged
        case .failure(let error):
    ...
    func cancelVote() async {
        ...
        if case .failure(let error) = await votingUseCase.cancelVote(votingId: votingIdData.votingId) {
            handleError(error: error)
        }
    }

    func createFavorite() async {
        if case .failure(let error) = await favoriteUseCase.createFavoritePoll(pollHashId: id) {
            handleError(error: error)
        }
    }

    func deleteFavorite() async {
        if case .failure(let error) = await favoriteUseCase.deleteFavoritePoll(pollHashId: id) {
            handleError(error: error)
        }
    }

    func getFavorite() async {
        if case .failure(let error) = await favoriteUseCase.getFavoritePolls() {
            handleError(error: error)
        }
    }

    func reportPoll() async {
        switch await votingUseCase.reportPoll(pollHashId: id, content: reportReason, reportType: reportType) {
        case .success:
